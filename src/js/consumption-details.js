import { getConsumptionDetails } from "../api/get-consumption-detail";
...
const $consumptionDetailsList = document.querySelector(
 ...
 export const handleGetConsumptionDetails = async () => {
   ...
   const list = await getConsumptionDetails(); // list 데이터 ,
 
   list.map(({ createdAt, category, description, price, fundsAtTheTime }) => {
     ...
     // 날짜
     $pDate.textContent = new Date(createdAt).toLocaleDateString();
     ...
     // 캡션
     $consumptionDetailsItemCaption.textContent = `남은 자산 ${fundsAtTheTime}원`;
